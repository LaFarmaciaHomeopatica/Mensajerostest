...
echo "=== ACTUALIZANDO PREGUNTAS PREOPERACIONALES PARA MOTOS ===\n\n";

// Primero, eliminar los ítems que NO son para motos
$toDelete = ['cinturon', 'kit_carreteras'];
DB::table('preoperational_questions')->whereIn('key', $toDelete)->delete();
echo "Eliminadas preguntas no aplicables (cinturón, kit carreteras).\n";

// Ahora insertar o actualizar las preguntas de moto
$questions = [
...
foreach ($questions as $question) {
    DB::table('preoperational_questions')->updateOrInsert(
        ['key' => $question['key']],
        array_merge($question, ['updated_at' => now(), 'created_at' => now()])
    );
}
...
// Mostrar resumen
$summary = DB::table('preoperational_questions')
...
foreach ($summary as $s) {
    echo "{$s->category}: {$s->total} preguntas\n";
    $total += $s->total;
}
